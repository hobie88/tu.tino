...
class default(CommonQboState):
    def __init__(self):
        ...
        smach.State.__init__(self, outcomes=['mplayer','questions','webi',''])
        self.state="Default"

        self.input_values={"RUN MUSIC PLAYER":"mplayer", "RUN WEB INTERFACE":"webi","LAUNCH CHAT MODE":"questions"}
...
class musicplayer(CommonQboState):
    def __init__(self):
        ...
        self.input_values={"STOP MUSIC PLAYER":"stop", "STOP STATE MACHINE":"exit"}
...
def main():
    ...
    with sm:
        smach.StateMachine.add('default', default(), transitions={'mplayer':'music_player','webi':'webi','questions':'questions','':'exit'})
        smach.StateMachine.add('music_player', musicplayer(), transitions={'stop':'default', '':'exit'})
